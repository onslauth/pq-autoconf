...
pq_part_name := autoconf-2.69
pq_part_file := $(pq_part_name).tar.xz

pq_autoconf_configuration_flags += --prefix=$(part_dir)
...
configure-stamp: patch-stamp
	cd $(pq_part_name) && ./configure $(pq_autoconf_configuration_flags)
	touch $@
